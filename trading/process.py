...
def log_transaction(transaction_type, date, stock, number_of_shares, price, fees, ledger_file):
    ...
    '''
    with open(ledger_file, 'a') as mynewfile:
        if number_of_shares!=0:
            if transaction_type=='buy':
                mynewfile.write(f'{transaction_type},{date},{stock},{number_of_shares},{np.around(price,2)},{np.around(-number_of_shares*price-fees,2)}'+'\n')
...
def buy(date, stock, available_capital, stock_prices, fees, portfolio, ledger_file):
    ...
    if np.isnan(price)==False:
        num_buy=math.floor((available_capital-fees)/price)
        if num_buy!=0: # we can only but a stock when the number of stock we can afford is a positive number
            portfolio[stock]+=num_buy
            log_transaction('buy', date, stock, num_buy, price, fees,ledger_file)
    return None
...
def sell(date, stock, stock_prices, fees, portfolio, ledger_file):
    ...
    price=stock_prices[date,stock]
    if np.isnan(price)==False:     
        if portfolio[stock]!=0: # we can only sell a stock when its current portfolio is positive
            num_sell=portfolio[stock]
            portfolio[stock]=0
            log_transaction('sell', date, stock, num_sell, price, fees, ledger_file)
    return None



def create_portfolio(available_amounts, stock_prices, fees, ledger='ledger.txt'):
    '''
    ...
    for i in range(N):
        buy(0, i, available_amounts[i], stock_prices, fees, portfolio, ledger)      
    return portfolio
